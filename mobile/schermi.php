...
A.sc:hover {text-decoration: none; color: black;}
  ...
  ?>
    ...
    <div align=center>  
      <br>  Clicca sul nome dello schermo per modificarne i parametri   
      <table cellpadding="10">  
        ...
        <td><b>Stato</b></td>  
            ...
            <?php
              $data = Screen::list_all();
              $lunghezza = count($data);
            for($i=0; $i < $lunghezza; $i++){
            $str = "WHERE id=".$data[$i]['id'];
            $schermo = Screen::screenStats($str);
            if($schermo[0] == 1){
                $status = "Online";
            		$statcolor = "green";
            }else
            ...
            if($schermo[2] == 1){
                    ...
                    ?>
        <tr align=center>
        <td><a href="show.php?id=<?= $data[$i]['id']; ?>" class="sc"><? echo $data[$i]['name'] ?></a></td>	
        <td><? echo $data[$i]['location'] ?></td>	 
          <td style="color:<? echo $statcolor ?>;"><? echo $status ?></td>
        </tr>	
