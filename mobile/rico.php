...
function crea_rico(rico, tipo, col){

if(rico == "0" || col == "0" || tipo == "0")
          document.getElementById("feedrico").value='<?= $rico_6; ?>';
else{
    if(rico == "rico") 
         document.getElementById("feedrico").value='<iframe width="450" height="320" scrolling="no" src="<?= ROOT_URL ?>wiki.php?tipo='+tipo+'&col='+col+'" frameborder="0"></iframe>';
    if(rico == "nati") 
         document.getElementById("feedrico").value='<iframe width="450" height="320" scrolling="no" src="<?= ROOT_URL ?>nati.php?tipo='+tipo+'&col='+col+'" frameborder="0"></iframe>';    
    if(rico == "morti") 
         document.getElementById("feedrico").value='<iframe width="450" height="320" scrolling="no" src="<?= ROOT_URL ?>morti.php?tipo='+tipo+'&col='+col+'" frameborder="0"></iframe>';    
  }                     
}
</script>
  </head>  
  ...
  ?>
    ...
    <div align=center> 
      <form ENCTYPE="multipart/form-data" method="post" action="add_rico.php">
      <b>Aggiungi Ricorrenze</b><br><br>
      Tipo<br>
      <select name="rico" id="rico" onChange="crea_rico(this.options[this.selectedIndex].value, tipo.options[tipo.selectedIndex].value, coloretesto.options[coloretesto.selectedIndex].value);">
    <option value="0"> </option>
    <option value="rico">Ricorrenze</option>
    <option value="nati">Nati del giorno</option>
    <option value="morti">Morti del giorno</option>
    </select><br>
    Carattere<br>
    <select name="tipo" id="tipo" onChange="crea_rico(rico.options[rico.selectedIndex].value, this.options[this.selectedIndex].value, coloretesto.options[coloretesto.selectedIndex].value);">
    <option value="0"></option>
    ...
    Colore<br>
    <select name="coloretesto" id="coloretesto" onChange="crea_rico(rico.options[rico.selectedIndex].value, tipo.options[tipo.selectedIndex].value, this.options[this.selectedIndex].value);">
    <option value="0"> </option>
    ...
    <div style="DISPLAY: none">
      <textarea name="contenuto" id="feedrico" rows="4" cols="40"></textarea>
      </div> 
      Titolo<br>
